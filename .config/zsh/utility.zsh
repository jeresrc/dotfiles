...
function _zv(){ 
    if ! command -v zoxide &> /dev/null; then 
        echo "zoxide is not installed. Please install zoxide (https://github.com/ajeetdsouza/zoxide) and try again."; 
        return 1; 
    fi; 
    
    if [ "$#" -ne 1 ]; then 
        echo "Usage: zv <directory>"; 
        return 1; 
    fi; 
    
    directory="$1"; 
    zoxide_cd="$(zoxide query "$directory")"; 
    
    if [ -z "$zoxide_cd" ]; then 
        echo "Directory not found in zoxide database."; 
        return 1; 
    fi; 
